...
export function MenuManager({ subdomain }: MenuManagerProps) {
  ...
  const fetchMenuItems = async () => {
    setIsLoading(true)
    try {
      const response = await fetch(`/api/${subdomain}/menu`)
    ...
    <div className="container mx-auto p-4 max-w-6xl">
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6">
        ...
        <div className="flex gap-2">
          <Select
            value={filterDietary || "all"}
            onValueChange={(value) => setFilterDietary(value === "all" ? null : value)}
          >
        ...
        <div className="space-y-6">
          {filteredCategories?.map((category) => (
            <div key={category.id} className="bg-white rounded-lg p-4 mb-8">
              ...
              <div key={`items-${category.id}`} className="space-y-4">
