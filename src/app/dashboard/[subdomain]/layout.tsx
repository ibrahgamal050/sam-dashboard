...
export default function RestaurantAdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const { subdomain } = useParams() as { subdomain: string | string[] }
  const resolvedSubdomain = Array.isArray(subdomain) ? subdomain[0] : subdomain
  ...
  // افتح السايدبار تلقائياً على الشاشات الكبيرة فقط
  useEffect(() => {
  ...
  }, [pathname, resolvedSubdomain, router, searchParamsString])
