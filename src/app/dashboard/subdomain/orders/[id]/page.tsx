...
export default async function OrderDetailsPage({
    ...
    return {
      ...
      items: Array.isArray(order.items)
        ? order.items.map((item: any) => ({
            ...
            name: resolveText(item.name) || "عنصر",
  ...
  const normalizeRetailOrder = (order: any): NormalizedOrder => {
    const items: OrderItemLean[] = Array.isArray(order.items)
      ? order.items.map((item: any) => {
          ...
          return {
            _id: item._id,
            productId: item.productId,
            name: resolveText(item.name) || "منتج",
            quantity: qty,
    ...
    return {
      ...
      items,
      customer: {
        name: resolveText(order.customer?.name) || "عميل السوبرماركت",
  ...
  const customerName = resolveText(customer.name) || "عميل"
  ...
  const storeTypeLabel = order.source === "restaurant" ? "مطعم" : "سوبرماركت"
  ...
  const orderTypeLabel =
    order.type === "Pickup" ? "استلام ذاتي" : order.type === "Dine-in" ? "داخل المطعم" : "توصيل"

  const timeline = [
    { label: "تم إنشاء الطلب", value: createdAt },
    eta ? { label: "الوقت المتوقع", value: eta } : null,
      ...
      <div className="space-y-6">
          ...
          <CardHeader className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="space-y-3">
              <div className="flex flex-wrap items-center gap-2 text-sm text-slate-500">
                <Link
                  href={`/dashboard/${subdomain}/orders`}
                ...
                <span className="rounded-full bg-slate-100 px-3 py-1">طلب #{orderReference}</span>
              ...
              <div className="flex flex-wrap items-center gap-3">
                <div>
                  ...
                  <p className="text-sm text-slate-500">الفرع: {storeName}</p>
                </div>
                <span
                  className={cn(
              ...
              <p className="text-sm text-slate-500">تاريخ الإنشاء {createdAt}</p>
            </div>
            <div className="grid gap-3 text-sm text-slate-600 sm:grid-cols-2">
              <div className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2">
                <Truck className="h-4 w-4 text-slate-500" />
                <div>
                  <p className="text-xs uppercase tracking-[0.15em] text-slate-400">الحالة</p>
              ...
              <div className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2">
                ...
                <div>
                  <p className="text-xs uppercase tracking-[0.15em] text-slate-400">الإجمالي</p>
              ...
              <div className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2">
                ...
                <div>
                  <p className="text-xs uppercase tracking-[0.15em] text-slate-400">العناصر</p>
              ...
              <div className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2">
                ...
                <div>
                  <p className="text-xs uppercase tracking-[0.15em] text-slate-400">الهاتف</p>
        ...
        <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
          <div className="space-y-6">
            <Card className="border border-slate-200 bg-white shadow-[0_18px_40px_rgba(15,23,42,0.06)]">
              <CardHeader className="flex flex-row items-center justify-between">
                <CardTitle className="text-base font-semibold text-slate-900">العناصر</CardTitle>
            ...
            <Card className="border border-slate-200 bg-white shadow-[0_18px_40px_rgba(15,23,42,0.06)]">
              <CardHeader>
                <CardTitle className="text-base font-semibold text-slate-900">المخطط الزمني</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
          ...
          <div className="space-y-6">
            <Card className="border border-slate-200 bg-white shadow-[0_18px_40px_rgba(15,23,42,0.06)]">
              <CardHeader>
                <CardTitle className="text-base font-semibold text-slate-900">العميل</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4 text-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-slate-900">{customerName || "ضيف"}</p>
            ...
            <Card className="border border-slate-200 bg-white shadow-[0_18px_40px_rgba(15,23,42,0.06)]">
              <CardHeader>
                <CardTitle className="text-base font-semibold text-slate-900">الدفع</CardTitle>
              </CardHeader>
            ...
            {order.notes && (
              <Card className="border border-slate-200 bg-white shadow-[0_18px_40px_rgba(15,23,42,0.06)]">
                <CardHeader>
                  <CardTitle className="text-base font-semibold text-slate-900">ملاحظة العميل</CardTitle>
                </CardHeader>
                <CardContent>
            ...
            <Card className="border border-slate-200 bg-white shadow-[0_18px_40px_rgba(15,23,42,0.06)]">
              <CardHeader>
                <CardTitle className="text-base font-semibold text-slate-900">بيانات إضافية</CardTitle>
              </CardHeader>
