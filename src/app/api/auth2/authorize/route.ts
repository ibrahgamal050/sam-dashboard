...
export async function GET(request: Request) {
  ...
  if (normalizedBrandSlug) {
    const brand = await Brand.findOne({ slug: normalizedBrandSlug }).lean<{ _id: mongoose.Types.ObjectId } | null>();
    if (!brand?._id) {
      return NextResponse.json({ error: "NOT_FOUND" }, { status: 404 });
    }

    const authorized = isOwnerForTarget(user, {
      type: "brand",
      id: brand._id.toString(),
      brandId: brand._id.toString(),
    });

    console.log("[/api/auth/authorize] brand auth check", {
    ...
    return NextResponse.json({
      ok: true,
      target: { type: "brand", id: brand._id.toString(), brandId: brand._id.toString() },
    });
  }
