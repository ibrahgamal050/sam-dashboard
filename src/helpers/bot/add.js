...
async function procesrestaurantData(nameEn, nameAr) {
    ...
    const response = await fetch("https://api.perplexity.ai/search", {
        ...
        body: JSON.stringify({
            query: `ابحث عن معلومات حول مطعم ${nameAr}, اعد المحتوي في صيغة كود جسون كما بالمثال const Data = {
                subdomain: "test",
                nameEn: "test",
                nameAr: "مطعم بيتزا السلطان",
                cuisineEn: "Italian",
...
async function processMenuData(arData, enData) {
  try {
...
async function main() {
 const restaurantData= await procesrestaurantData (nameEn, nameAr);
 ...
 await makeRequest(`/${restaurantData.subdomain}/pages`, pagesData);
  ...
  const processedMenu = await processMenuData(arData, enData);
 await makeRequest(`/${restaurantData.subdomain}/menu`, processedMenu);
}
